-- 1. 집계쿼리: select 절에 통계함수(avg, max, min, count, sum, stddev, varance, ...)alter
select avg(salary), sum(salary)
	from salaries;

-- 2. select 절에 그룹함수(통계함수) 있는 경우, 어떤 컬럼도 select절에 올 수 없음
-- emp_no는 아무 의미가 없음
select emp_no, avg(salary)
	from salaries;   -- 오류!!! MySQL은 오류내지않음
