...
-- 예) 2014년 07월 10일
select date_format(min(from_date), '%Y년 %m월 %d일') as '가장 오래 근속한 직원의 입사일' 
	from titles
...
select date_format(now(), '%Y') - date_format(max(birth_date), '%Y') as '어린 사원',
	date_format(now(), '%Y') - date_format(min(birth_date), '%Y') as '최고 연장자'
    from employees;
